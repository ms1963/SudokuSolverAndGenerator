...
The Sudoku board is a 3x3-matrix of quadrants (Q stands for 
quadrant) with each quadrant defining a 3x3-matrix of numbers:
...
|Q3,1| |Q3,2| |Q3,3|
    ...
    Entries of array:
    Tuples (x, y, z)
    where x is True  iff location is occupied by number y
          x is false iff location is vacant. z defines the
          numbers which influence this position, i.e., numbers
          which can not occupy this position and are therefore
          no candidates
...
SudokuSolver. 
An influencer is a number that cannot occupy a cell <=>
the number is not a candidate.
OccupationStrategies help to find cells that can be occupied.
They are called within the method canbeOccupied() of
the SudókuSolver class. 

InfluenceStrategies help reduce the number of candidates for cells, 
thus assisting the occupation strategies.
They are called within the method occupy() of the 
SudokuSolver class.

Both go in hand in hand for solving a puzzle.
...
To add your own influence strategy: 
    ...
    own strategy and ...
    ... assign it to self.board (Dependency Injection)
For examples see IndirectInfluencersStrategy and XWingStrategy,
SwordfishStrategy in the code.
